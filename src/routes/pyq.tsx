...
import {
  BookOpen, Search, Filter, Zap, ChevronRight,
  CheckCircle2, XCircle, Loader2, Sparkles, RotateCcw,
  Trophy, Brain,
} from "lucide-react";
...
export const Route = createFileRoute("/pyq")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Previous Year Questions for JEE, NEET and more — practice or generate AI-powered PYQ-style questions" },
    ],
  }),
  component: PYQPage,
});
...
function PYQPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const qs: Omit<PYQQuestion, "id">[] = (result.questions || []).map((q: any) => ({
        exam_id: examId,
        ...
        question_type: "mcq",
        options: q.options,
        answer: q.options?.[q.answerIndex] || q.answer || "",
  ...
  const handlePractice = () => {
    ...
    setPracticing({ questions: [...questions].sort(() => Math.random() - 0.5).slice(0, 10), current: 0, selected: null, revealed: false, score: 0, done: false });
  };
    ...
    <div className="min-h-screen p-4 md:p-8 max-w-5xl mx-auto">
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Previous Year Questions · AI-powered generation</p>
          </div>
        </div>
        {questions.length > 0 && (
      ...
      <div className="glass-card rounded-2xl p-4 mb-6 border border-primary/20">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          />
          <Button onClick={handleGenerate} disabled={generating} className="bg-gradient-primary gap-2">
      ...
      <div className="glass-card rounded-2xl p-4 mb-6 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {/* Exam */}
          {EXAMS.map((e) => (
            <button key={e} onClick={() => { setExamId(e); setSubject(""); }}
              className={cn("px-3 py-1 rounded-full text-xs border transition-all",
                examId === e ? "bg-primary/15 text-primary border-primary/30" : "glass-subtle text-muted-foreground hover:text-foreground")}>
              {EXAM_LABELS[e]}
            </button>
          ))}
        </div>
        <div className="flex flex-wrap gap-2">
          {/* Subject */}
          <button onClick={() => setSubject("")} className={cn("px-3 py-1 rounded-full text-xs border transition-all",
            !subject ? "bg-primary/15 text-primary border-primary/30" : "glass-subtle text-muted-foreground")}>All subjects</button>
          {(SUBJECTS[examId] || []).map((s) => (
            <button key={s} onClick={() => setSubject(s === subject ? "" : s)}
              className={cn("px-3 py-1 rounded-full text-xs border transition-all",
                subject === s ? "bg-primary/15 text-primary border-primary/30" : "glass-subtle text-muted-foreground hover:text-foreground")}>
              {s}
            </button>
          ))}
        </div>
          ...
          {DIFFICULTIES.map((d) => (
            <button key={d} onClick={() => setDifficulty(d === difficulty ? "" : d)}
              className={cn("px-3 py-1 rounded-full text-xs border transition-all capitalize",
                difficulty === d
                  ? d === "hard" ? "bg-red-500/15 text-red-400 border-red-500/30"
      ...
      {/* Question list */}
      {isLoading ? (
        <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-20 glass-card rounded-xl animate-pulse" />)}</div>
      ) : questions.length === 0 ? (
        <div className="glass-card rounded-2xl p-12 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-6">Generate some using the AI tool above, or adjust your filters.</p>
        </div>
      ) : (
...
function QuestionCard({ question: q, index }: { question: PYQQuestion; index: number }) {
  ...
  const diffColor = q.difficulty === "hard" ? "text-red-400 bg-red-500/10 border-red-500/20"
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap gap-1.5 mb-2">
            <span className="text-xs text-muted-foreground">{q.exam_id?.toUpperCase()} · {q.year} · {q.subject}</span>
            <span className={cn("text-[10px] px-2 py-0.5 rounded-full border capitalize", diffColor)}>{q.difficulty}</span>
          </div>
          <p className="text-sm leading-relaxed">{q.question}</p>
          {expanded && q.options && (
            <div className="mt-3 space-y-1.5">
              {q.options.map((opt, oi) => (
                <div key={oi} className={cn("text-sm px-3 py-2 rounded-lg border",
                  opt === q.answer ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400" : "glass-subtle")}>
                  {opt === q.answer && <CheckCircle2 className="inline h-3.5 w-3.5 mr-1.5" />}{opt}
                </div>
              ))}
              {q.explanation && <p className="text-xs text-muted-foreground mt-2 p-3 bg-muted/20 rounded-lg">{q.explanation}</p>}
            </div>
          )}
          <button onClick={() => setExpanded((e) => !e)} className="mt-2 text-xs text-primary hover:underline">
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="glass-card rounded-2xl p-10 text-center max-w-md w-full">
          ...
          <p className="text-muted-foreground mb-6">{state.score}/{total} correct · {pct}% accuracy</p>
          ...
          <div className="flex gap-3 justify-center">
            <Button variant="outline" onClick={() => setState({ ...state, current: 0, selected: null, revealed: false, score: 0, done: false })} className="gap-2">
              <RotateCcw className="h-4 w-4" /> Try again
            </Button>
            <Button onClick={() => setState(null)} className="bg-gradient-primary">Back to bank</Button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen p-4 md:p-8 max-w-2xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <div className="h-1.5 bg-muted/30 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-primary rounded-full transition-all" style={{ width: `${((state.current) / total) * 100}%` }} />
        </div>
      </div>

      <div className="glass-card rounded-2xl p-6 space-y-5">
        <div className="flex gap-2 text-xs text-muted-foreground">
          <span>{q.exam_id?.toUpperCase()}</span><span>·</span><span>{q.year}</span><span>·</span><span>{q.subject}</span>
        </div>
        ...
        <div className="space-y-2">
          {(q.options || []).map((opt, idx) => {
            ...
            if (state.revealed) {
              cls = isCorrect ? "bg-emerald-500/15 border-emerald-500/40 text-emerald-300"
            ...
            return (
              <button key={idx} onClick={() => handleSelect(idx)} disabled={state.revealed}
                className={cn("w-full text-left px-4 py-3 rounded-xl transition-all flex items-center gap-3", cls)}>
                {state.revealed && isCorrect && <CheckCircle2 className="h-4 w-4 shrink-0" />}
                {state.revealed && isSelected && !isCorrect && <XCircle className="h-4 w-4 shrink-0" />}
                <span>{opt}</span>
              </button>
            );
          })}
        ...
        {state.revealed && (
          <Button onClick={handleNext} className="w-full bg-gradient-primary">
            {state.current + 1 >= total ? "See Results" : "Next Question"} <ChevronRight className="h-4 w-4 ml-1" />
          </Button>
        )}
      </div>
    </div>
