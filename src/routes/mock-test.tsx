...
import {
  Clock, ChevronLeft, ChevronRight, Flag, Loader2, Play,
  CheckCircle2, XCircle, AlertTriangle, BarChart3, RotateCcw,
  BookOpen, Sparkles, Timer, CircleDot, Pause,
} from "lucide-react";
...
export const Route = createFileRoute("/mock-test")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Take timed, exam-pattern mock tests generated by AI to simulate the real exam." },
    ],
  }),
  ...
  const handleGenerate = async () => {
    ...
    try {
      const sections = examInfo.subjects
        .filter((s) => !profile?.selected_subjects?.length || profile.selected_subjects.includes(s.name))
        .map((s) => ({
          name: s.name,
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <div className="text-sm text-muted-foreground">
              {examInfo ? `${examInfo.icon} ${examInfo.name} pattern` : "Complete onboarding to start"}
            </div>
          </div>
        </div>
          ...
          <>
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="p-3 rounded-xl bg-muted/30 text-center">
                <div className="text-lg font-bold font-heading">{examInfo.examPattern.sections.length}</div>
                <div className="text-xs text-muted-foreground">Sections</div>
              </div>
              <div className="p-3 rounded-xl bg-muted/30 text-center">
                <div className="text-lg font-bold font-heading">{examInfo.examPattern.totalTimeMinutes}m</div>
                <div className="text-xs text-muted-foreground">Total time</div>
              </div>
              <div className="p-3 rounded-xl bg-muted/30 text-center">
                <div className="text-lg font-bold font-heading">{examInfo.examPattern.markingScheme.split(".")[0]}</div>
                ...
                <div key={test.id} className="glass-card p-4 rounded-2xl flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground flex items-center gap-2 mt-0.5">
                      <span>{test.score}/{test.total_marks} marks</span>
                      <span>·</span>
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <header className="glass border-b border-border px-4 py-3 flex items-center justify-between sticky top-0 z-30">
        ...
        <div className={cn(
          ...
          isTimeLow ? "bg-destructive/15 text-destructive animate-pulse" : "bg-primary/10 text-primary",
        )}>
              ...
              <div className="grid grid-cols-5 gap-1.5">
                {section.questions.map((q, qi) => {
                  ...
                  return (
                    <button
                      key={q.id}
                      onClick={() => { setActiveSection(si); setActiveQuestion(qi); }}
                      className={cn(
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          realIdx === activeQuestion
                            ? "bg-gradient-primary text-white"
                            : isAns ? "bg-success/15 text-success" : "bg-muted/30 text-muted-foreground",
                        )}
                      >
                        {realIdx + 1}
                      </button>
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowConfirm(false)}>
          <div className="glass p-6 rounded-2xl max-w-sm w-full shadow-elegant" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
...
function ResultScreen({
  ...
  const incorrect = allQuestions.filter(
    (q) => test.answers[q.id] !== null && test.answers[q.id] !== undefined && test.answers[q.id] !== q.answerIndex,
  ).length;
    ...
    <div className="max-w-4xl mx-auto px-6 py-10">
      ...
      <div className="glass-card p-8 rounded-2xl text-center mb-6">
        ...
        <p className="text-muted-foreground">
          You scored <strong className="text-foreground">{test.score}/{test.total_marks}</strong> marks in{" "}
          <strong className="text-foreground">{mins} minutes</strong>
        </p>
      </div>
      ...
      <div className="glass-card p-5 rounded-2xl mb-6">
        ...
        <div className="space-y-3">
          {test.sections.map((section) => {
            const sCorrect = section.questions.filter((q) => test.answers[q.id] === q.answerIndex).length;
            const sPct = section.questions.length ? Math.round((sCorrect / section.questions.length) * 100) : 0;
            return (
              <div key={section.name}>
                <div className="flex items-center justify-between text-sm mb-1">
                  <span>{section.name}</span>
                  <span className="font-medium">{sCorrect}/{section.questions.length} ({sPct}%)</span>
                </div>
                ...
                <div key={q.id} className="glass-card p-5 rounded-2xl">
                  <div className="flex items-center gap-2 mb-3">
                    <span className={cn(
                      ...
                      isCorrect ? "bg-success/10 text-success" : isUnanswered ? "bg-muted/30 text-muted-foreground" : "bg-destructive/10 text-destructive",
                    )}>
                      {isCorrect ? <CheckCircle2 className="h-3.5 w-3.5" /> : isUnanswered ? "−" : <XCircle className="h-3.5 w-3.5" />}
                    </span>
